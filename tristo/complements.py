...
PATH_WVG = os.path.join(PATH_SUPP,'WW_Analysedaten_10-2021.xlsx')
WVG_list = pd.read_excel(PATH_WVG,
                    usecols=[0, 1, 2, 3, 4], index_col=[1]).sort_values(by='Versorgte Bevölkerung',ascending=False)

PATH_NUTS = os.path.join(PATH_SUPP,'EU-27-LAU-2021-NUTS-2021.xlsx')
         
settings={'names':['NUTS', 'LAU', 'full_name', 'population', 'area_m2']}
LAU_NUTS = {2021: pd.read_excel(PATH_NUTS, sheet_name='DE',usecols=[0,1,2,5,6], **settings)}